  ...
  // $i=0;
   echo "";
      ...
      if(isset($_REQUEST['d1']))
		 {
			$result = mysqli_query($con,"SELECT * FROM $table WHERE purchase_date BETWEEN '$_REQUEST[d1]' AND '$_REQUEST[d2]' AND pm_id='$_REQUEST[id]' "); 
		 }
		 else
		 {
         
 	$result = mysqli_query($con,"SELECT * FROM $table pm_id='$_REQUEST[id]' ");
		 }
